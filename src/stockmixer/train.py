...
# from stockmixer.load_data import load_EOD_data
from stockmixer.evaluator import evaluate
from stockmixer.model import get_loss, StockMixer
import pickle
...
dataset_path = f"{data_path}/{market_name}"
if market_name == "SP500":
    data = np.load(f"{data_path}/SP500/SP500.npy")
    data = data[:, 915:, :]
    price_data = data[:, :, -1]
